...
import static com.github.iotexproject.antenna.solidity.SolidityType.IntType.decodeInt;
import static com.github.iotexproject.antenna.solidity.SolidityType.IntType.encodeInt;
import static com.github.iotexproject.antenna.utils.ArrayUtils.subarray;
import static com.github.iotexproject.antenna.utils.Strings.join;
import static com.github.iotexproject.antenna.utils.Strings.stripEnd;
...
public class Abi extends ArrayList<Abi.Entry> {
    ...
    public Function findFunction(String name) {
        for (Entry entry : this) {
            if (entry.type == Entry.Type.function && entry.name.equals(name)) {
    ...
    public Event findEvent(String name) {
        for (Entry entry : this) {
            if (entry.type == Entry.Type.event && entry.name.equals(name)) {
                return (Event) entry;
            }
        }
        return null;
    ...
    public Constructor findConstructor() {
        for (Entry entry : this) {
            if (entry.type == Entry.Type.constructor) {
    ...
    public static class Function extends Entry {
        ...
        public byte[] encode(Object... args) {
            return Numeric.merge(encodeSignature(), encodeArguments(args));
        }
